# Publier KilomPro sur le Google Play Store

KilomPro est une PWA (Progressive Web App). Pour la publier sur le Play Store, on l'empaquette dans une **TWA** (Trusted Web Activity) : une coque Android minimale qui affiche le site en plein écran, sans barre de navigateur. L'appli continue de charger le contenu réel depuis GitHub Pages.
...
- `packageId` : identifiant unique de l'appli (ex. `com.tonnom.kilompro`), format inversé de nom de domaine. **Impossible à changer après la première publication**, choisis-le avec soin.
...
```bash
keytool -list -v -keystore android.keystore -alias kilompro
```
